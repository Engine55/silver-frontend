    ...
    def get_online_users(self, exclude_user: str = None):
        ...
        for user_id, user_info in self.users.items():
            if user_id != exclude_user and user_info["status"] == "online":
                online_users.append({"id": user_id, "status": user_info["status"]})
        return online_users
        ...
        while True:
            ...
            if message["type"] == "ice_candidate":
                ...
                if "call_id" in message:
                    await manager.answer_call(message["call_id"], True, message["answer"])
                else:
                    target = message.get("target")
                    if target:
                        await manager.send_to_user(target, {
...
async def join_room(request: JoinRoomRequest):
    result = await manager.join_room(request.userId, request.roomId, request.offer.dict())
    return result

@app.post("/api/call-user")
async def call_user(request: CallUserRequest):
    result = await manager.call_user(request.from_user, request.to, request.offer.dict())
    return result
