 ...
 * 连接状态转换为中文
 */
export function translateConnectionState(state: RTCPeerConnectionState): string {
  switch (state) {
    case 'new':
      return '新建连接';
    case 'connecting':
 ...
 * ICE 连接状态转换为中文
 */
export function translateIceConnectionState(state: RTCIceConnectionState): string {
  switch (state) {
    case 'new':
      return 'ICE新建';
    case 'checking':
...
export function createWebSocketWithRetry(
  ...
  return new Promise((resolve, reject) => {
    ...
    function connect() {
      ...
      ws.onerror = (error) => {
        console.error(`❌ WebSocket 连接错误 (尝试 ${retryCount + 1}/${maxRetries + 1}):`, error);
      ...
      ws.onclose = (event) => {
        if (!event.wasClean && retryCount < maxRetries) {
          console.log(`🔄 WebSocket 连接意外关闭，${retryDelay * (retryCount + 1)}ms 后重试...`);
          retryCount++;
          setTimeout(connect, retryDelay * retryCount);
        }
      };
 ...
 */
export async function logPeerConnectionStats(pc: RTCPeerConnection): Promise<void> {
  try {
    ...
    stats.forEach((report) => {
      ...
      if (report.type === 'outbound-rtp' && report.mediaType === 'video') {
        if (report.bytesSent && report.timestamp) {
          // 简单的带宽估算
          result.bandwidth = report.bytesSent * 8 / (report.timestamp / 1000);
        }
      }
    });
